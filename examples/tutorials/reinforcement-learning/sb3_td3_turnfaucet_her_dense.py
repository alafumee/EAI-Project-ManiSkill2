...
class TurnFaucetHERWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        new_ob.update(
            desired_goal=np.array([self.env.target_angle]),
            achieved_goal=np.array([self.env.current_angle]),
        )
        """Compute the distance between the tap and robot fingers."""
        T = self.env.target_link.pose.to_transformation_matrix()
        pcd = transform_points(T, self.env.target_link_pcd)
        T1 = self.env.lfinger.pose.to_transformation_matrix()
        T2 = self.env.rfinger.pose.to_transformation_matrix()
        pcd1 = transform_points(T1, self.env.lfinger_pcd)
        pcd2 = transform_points(T2, self.env.rfinger_pcd)
        # trimesh.PointCloud(np.vstack([pcd, pcd1, pcd2])).show()
        distance1 = cdist(pcd, pcd1)
        distance2 = cdist(pcd, pcd2)
        tcp_tap_dist = min(distance1.min(), distance2.min())
        
        # is_grasped = self.env.agent.check_grasp(self.env.obj, max_angle=30)
        # tcp_pos = self.env.tcp.pose.p
        info.update(
            real_target=self.env.target_angle,
            last_angle_diff=self.env.last_angle_diff,
            tcp_tap_dist=tcp_tap_dist,
        )
        ...
        #     np.array([info]),
        #     )
        # assert abs(recons_rew[0][0] - rew) < 1e-7, f"Rewards are not equal! rew={rew}, recons_rew={recons_rew[0][0]}, obs: {new_ob}"
    ...
    def reset(self, *args, **kwargs):
        ...
        tcp_tap_dist = min(distance1.min(), distance2.min())
        info.update(
            real_traget=self.env.target_angle,
            last_angle_diff=self.env.last_angle_diff,
            tcp_tap_dist=tcp_tap_dist,
        )
        return new_ob, info
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        target_angle = desired_goal
        current_angle = achieved_goal
        success = target_angle - current_angle <= 0
        tcp_tap_dist = np.expand_dims(np.array(list(map(lambda x: x["tcp_tap_dist"], info))), axis=-1)
        
        reaching_reward = 1.0 - np.tanh(5 * tcp_tap_dist)
        turn_reward_1 = 3 * (1 - np.tanh(np.maximum(target_angle - current_angle, 0) * 2.0))
        # TODO rest of the rewards!
        angle_diff = np.expand_dims(np.array(list(map(lambda x: x["real_target"], info))), axis=-1) - current_angle
        delta_angle = angle_diff - np.expand_dims(np.array(list(map(lambda x: x["last_angle_diff"], info))), axis=-1)
        turn_rew_w_fn = lambda delta, ang_diff: - np.tanh(delta * 2.0) if ang_diff > 0 else np.tanh(delta * 2.0)
        vec_fn = np.vectorize(turn_rew_w_fn)
        turn_reward_2 = 5 * vec_fn(delta_angle, angle_diff)
        ...
        # reaching_reward = (1 - np.tanh(5 * np.linalg.norm(obj_pos - tcp_pos, axis=-1)))
        # place_reward = (1 - np.tanh(5 * np.linalg.norm(goal_pos - obj_pos, axis=-1)))
        
        rew = 10.0 * success + (reaching_reward + turn_reward_1 + turn_reward_2) * (1 - success)
...
def main():
    ...
    her_num_fn = lambda x: 0 # lambda x: 0.25 if x >= 250000 else 0
